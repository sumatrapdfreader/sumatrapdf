...
static int _unmarshal_pmgl_header(unsigned char** pData, unsigned int* pDataLen, unsigned int blockLen,
...
static int _unmarshal_pmgi_header(unsigned char** pData, unsigned int* pDataLen, unsigned int blockLen,
...
struct chmFile* chm_open(const char* d, size_t len) {
    ...
    sbufpos = sbuffer;
    if (_chm_fetch_bytes(newHandle, sbuffer, (UInt64)itsfHeader.dir_offset, sremain) != sremain ||
...
int chm_resolve_object(struct chmFile* h, const char* objPath, struct chmUnitInfo* ui) {
    ...
    while (curPage != -1) {
        ...
        if (_chm_fetch_bytes(h, page_buf, (UInt64)h->dir_offset + (UInt64)curPage * h->block_len, h->block_len) !=
            h->block_len) {
            free(page_buf);
            return CHM_RESOLVE_FAILURE;
        }
...
static int _chm_get_cmpblock_bounds(struct chmFile* h, UInt64 block, UInt64* start, Int64* len) {
    ...
    if (block < h->reset_table.block_count - 1) {
        ...
        if (_chm_fetch_bytes(h, buffer,
                             (UInt64)h->data_offset + (UInt64)h->rt_unit.start + (UInt64)h->reset_table.table_offset +
                                 (UInt64)block * 8,
                             remain) != remain ||
...
LONGINT64 chm_retrieve_object(struct chmFile* h, struct chmUnitInfo* ui, unsigned char* buf, LONGUINT64 addr,
    ...
    if (ui->space == CHM_UNCOMPRESSED) {
        /* read data */
        return _chm_fetch_bytes(h, buf, (UInt64)h->data_offset + (UInt64)ui->start + (UInt64)addr, len);
    }
