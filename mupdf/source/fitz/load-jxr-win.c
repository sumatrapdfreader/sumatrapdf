...
static fz_pixmap *
fz_load_jxr_or_info(fz_context *ctx, const unsigned char *data, size_t size, int *wp, int *hp, int *xresp, int *yresp, fz_colorspace **cspacep)
{
...
#undef Check
	...
	else
	{
		fz_try(ctx)
		{
			pix = fz_new_pixmap(ctx, fz_device_bgr(ctx), width, height, NULL, 0);
		}
		fz_catch(ctx)
		{
			pix = NULL;
