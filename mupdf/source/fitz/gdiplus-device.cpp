...
// Copyright (C) 2010 - 2014  Simon Bünzli <zeniko@gmail.com>
...
static fz_pixmap *
fz_new_pixmap_from_image_def(fz_context *ctx, fz_image *image, const fz_matrix *ctm)
{
	float dx = hypotf(ctm->a, ctm->b);
	float dy = hypotf(ctm->c, ctm->d);
	return fz_new_pixmap_from_image(ctx, image, dx, dy);
}

extern "C" static void
fz_gdiplus_fill_image(fz_device *dev, fz_image *image, const fz_matrix *ctm, float alpha)
{
	fz_pixmap *pixmap = fz_new_pixmap_from_image_def(dev->ctx, image, ctm);
	((userData *)dev->user)->started = true;
