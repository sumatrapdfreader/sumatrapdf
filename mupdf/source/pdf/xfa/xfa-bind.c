...
static void
pdf_xfa_bind_value(fz_context *ctx, fz_pool *pool, pdf_xfa_object *form, pdf_xfa_object *data)
{
	const char *value;

	form->data_node = data;
	if (!pdf_xfa_object_is_data_value(data))
		return;
	...
	if (!form->content || !form->content[0])
		form->content = fz_pool_strdup(ctx, pool, value);
}

static pdf_xfa_object *
pdf_xfa_find_data_child(pdf_xfa_object *data, const char *name, int skip_consumed)
{
	...
	{
		if (skip_consumed && child->consumed)
			continue;
...
static void
pdf_xfa_bind_element(fz_context *ctx, fz_pool *pool, pdf_xfa *xfa,
		pdf_xfa_object *form, pdf_xfa_object *data, int merge_mode);
		...
		pdf_xfa_object *form, pdf_xfa_object *data, int merge_mode)
{
	...
	for (child = form->first_child; child; child = next)
	{
		...
		{
			if (data && data->first_child)
				pdf_xfa_bind_element(ctx, pool, xfa, child, data->first_child, 0);
			continue;
		}
...
pdf_xfa_object *
pdf_xfa_bind(fz_context *ctx, fz_pool *pool, pdf_xfa *xfa)
{
	...
	if (!template_node)
		fz_throw(ctx, FZ_ERROR_FORMAT, "XFA: missing template packet");

	if (!data)
	{
		data = pdf_xfa_new_object(ctx, pool, PDF_XFA_NS_DATASETS, "data", 1);
		if (datasets)
			pdf_xfa_object_append_child(ctx, pool, datasets, data);
	}
	...
	if (!form)
		fz_throw(ctx, FZ_ERROR_GENERIC, "XFA: failed to clone template");
