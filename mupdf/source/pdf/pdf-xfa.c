...
int
pdf_document_has_xfa(fz_context *ctx, pdf_document *doc)
{
...
int
pdf_document_is_pure_xfa(fz_context *ctx, pdf_document *doc)
{
	return pdf_was_pure_xfa(ctx, doc);
}

void
pdf_xfa_drop_packets(fz_context *ctx, pdf_xfa_packet *packets)
{
	while (packets)
	{
...
pdf_xfa_packet *
pdf_xfa_load_packets(fz_context *ctx, pdf_document *doc)
{
	pdf_obj *xfa;
...
fz_buffer *
pdf_xfa_packets_to_xdp(fz_context *ctx, pdf_xfa_packet *packets)
{
	...
	{
		if (p->name && strcmp(p->name, "xdp:xdp") == 0)
			have_xdp = 1;
	}
	...
	for (p = packets; p; p = p->next)
	{
		if (!p->data || !p->name || strcmp(p->name, "xdp:xdp") == 0)
			continue;
...
pdf_xfa *
pdf_load_xfa(fz_context *ctx, pdf_document *doc)
{
	if (doc->xfa_ctx)
		return doc->xfa_ctx;
	...
	fz_catch(ctx)
	{
		fz_warn(ctx, "XFA: parse/bind failed");
		doc->xfa_ctx = NULL;
	}
...
int
pdf_xfa_is_valid(fz_context *ctx, pdf_xfa *xfa)
{
	(void)ctx;
...
int
pdf_xfa_layout(fz_context *ctx, pdf_xfa *xfa)
{
	return pdf_xfa_factory_layout(ctx, xfa);
}

int
pdf_xfa_page_count(fz_context *ctx, pdf_xfa *xfa)
{
	if (!xfa)
		return 0;
	if (!xfa->layout_done)
...
fz_rect
pdf_xfa_page_bbox(fz_context *ctx, pdf_xfa *xfa, int page_index)
{
	fz_rect empty = fz_empty_rect;
	if (!xfa || page_index < 0 || page_index >= pdf_xfa_page_count(ctx, xfa))
		return empty;
	return xfa->page_bboxes[page_index];
}

fz_display_list *
pdf_xfa_run_page(fz_context *ctx, pdf_xfa *xfa, int page_index, fz_matrix ctm)
{
...
fz_buffer *
pdf_xfa_serialize_data(fz_context *ctx, pdf_xfa *xfa)
{
