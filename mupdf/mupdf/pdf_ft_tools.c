...
static void scan_Lookup(struct ft2vert_st *ret, const FT_Bytes top)
{
	...
	if (ret->SubTableCount != SubTableCount)
		fz_warn("warning (scan_Lookup): SubTableCount (=%d) is not expected (=%d).\n",
...
static void scan_GSUB_Header(struct ft2vert_st *ret, const FT_Bytes top)
{
	...
	if (Version != 0x00010000)
		fz_warn("warning: GSUB Version (=%.1f) is not 1.0\n", (double)Version / 0x10000);
...
static struct ft2vert_st *ft2vert_init(FT_Face face)
{
	...
	{
		fz_warn("warning: %s has no GSUB table.\n", face->family_name);
		return ret;
	}
	scan_GSUB_Header(ret, ret->GSUB_table);
	if (ret->SubTableCount == 0)
		fz_warn("warning: %s has no vrt2/vert feature.\n", face->family_name);
...
int pdf_ft_get_vgid(pdf_fontdesc *fontdesc, int gid)
{
	int vgid = 0;
	if (!fontdesc->_vsubst)
		fontdesc->_vsubst = ft2vert_init(fontdesc->font->ftface);
...
void pdf_ft_free_vsubst(pdf_fontdesc *fontdesc)
{
	if (fontdesc && fontdesc->_vsubst)
		ft2vert_final(fontdesc->font->ftface, fontdesc->_vsubst);
}
