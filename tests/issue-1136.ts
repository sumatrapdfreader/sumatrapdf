...
import { getClassName, getFocusedHwnd, getWindowText, postMessage, sleep, waitForTopWindow } from "./winapi";
...
async function withHomePage(name: string, fn: (frame: number) => Promise<void>): Promise<void> {
    const proc = Bun.spawn([EXE, "-appdata", makeAppDir(name)], { stdout: "ignore", stderr: "ignore" });
    try {
        const frame = await waitForTopWindow(proc.pid, FRAME_CLASS);
        if (!frame) {
            throw new Error("no frame window");
        }
        await sleep(2500); // home page layout + first paint
        await fn(frame);
    } finally {
        proc.kill();
        await proc.exited;
...
export async function testit(): Promise<void> {
    ...
    await withHomePage("search", async (frame) => {
        ...
        if (!edit || getClassName(edit) !== "Edit") {
            throw new Error("could not focus the home page search box");
        }
