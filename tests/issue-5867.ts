...
async function runWithIcon(name: string, svg: string): Promise<void> {
    ...
    const proc = Bun.spawn([EXE, "-for-testing", "-appdata", dir, pdf], { stdout: "ignore", stderr: "ignore" });
    try {
        const frame = await waitForTopWindow(proc.pid, FRAME_CLASS);
        if (!frame) {
            throw new Error(`${name}: no frame window (exit code ${proc.exitCode})`);
        }
        // the toolbar bitmap is built during startup; give it a moment and make
