...
//
// Searching for text failed because mupdf text extraction dropped words set in
// embedded subset fonts (MSTT*) that name glyphs "G45", "G6D", ... (i.e. "G"
// + the hex character code) and ship no ToUnicode CMap, so mupdf couldn't map
// them to Unicode and emitted U+FFFD.
//
// The fixture is a minimal one-page PDF containing only the word "Embedded".
// Regenerate with: bun tests/issue-3219-make.ts
//
...
export async function testit(): Promise<void> {
  ...
  if (pdfjsText === null) {
    ...
    const pdfjsHas = pdfjsText.includes(TARGET);
    console.log(`pdf.js: ${pdfjsText.length} chars, has "${TARGET}": ${pdfjsHas}`);
    if (!pdfjsHas) {
      throw new Error(`pdf.js did not extract '${TARGET}' - unexpected, the fixture/baseline changed`);
    }
  }

  if (!mupdfHas) {
    throw new Error(`mupdf text extraction is missing '${TARGET}' (issue #3219 regressed)`);
  }
  console.log(`PASS: mupdf extracts '${TARGET}' (issue #3219)`);
}
