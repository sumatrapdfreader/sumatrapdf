...
// form with two pageAreas, per-page fields, and datasets data).
//
...
export async function testit(): Promise<void> {
  ...
  if (xfa.render_draws !== 1) {
    throw new Error(`ad-hoc-xfa.pdf: expected render_draws=1 on page 0, got render_draws=${xfa.render_draws}`);
  }
  if (xfa.p1_fields !== 1) {
    throw new Error(`ad-hoc-xfa.pdf: expected p1_fields=1 on page 1, got p1_fields=${xfa.p1_fields}`);
  }
  if (xfa.p1_draws !== 1) {
    throw new Error(`ad-hoc-xfa.pdf: expected p1_draws=1 on page 1, got p1_draws=${xfa.p1_draws}`);
  }
