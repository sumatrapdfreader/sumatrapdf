...
async function testLinkedPdf(): Promise<void> {
    ...
    await withControlledSumatra(
        ...
        async (client, proc) => {
            ...
            // the links near its top are on screen
            sendCommand(frame, cmdId("CmdZoomActualSize"));
            await sleep(500);
            // zooming keeps the view centered, so scroll back to the top of page 1
            sendCommand(frame, cmdId("CmdGoToFirstPage"));
            await sleep(800);
            ...
            if (!state.active) {
                fail("mode should be on after CmdToggleKeyboardLinkFollowing", dump);
            }
            ...
            if (state.active || state.count !== 0) {
                fail("second toggle should leave the mode", dump);
            }
            ...
            ({ state, dump } = await getState(client));
            if (!state.active) {
                fail("mode should be on before the Esc check", dump);
            }
            pressVKey(frame, VK_ESCAPE);
            await sleep(600);
            ({ state, dump } = await getState(client));
            if (state.active) {
                fail("Esc should leave the mode", dump);
            }
            ...
            await sleep(600);
            ({ state, dump } = await getState(client));
            if (!state.active || state.count !== 3) {
                fail("mode should be on again with 3 links", dump);
            }
            postMessage(frame, WM_CHAR, 0x32, 0);
            await sleep(900);
            ({ state, dump } = await getState(client));
            if (state.page !== 3) {
                fail(`pressing '2' should have followed the link to page 3, on page ${state.page}`, dump);
            }
            if (state.active) {
                fail("following a link should leave the mode", dump);
            }
        },
        [pdf],
    );
...
async function testRecomputeAfterScroll(): Promise<void> {
    ...
    await withControlledSumatra(
        ...
        async (client, proc) => {
            ...
            if (atTop !== 3) {
                throw new Error(`expected 3 links at the top, got ${atTop}\n${dump}`);
            }

            // scroll to the bottom of page 1, where only the 4th link is visible
            sendCommand(frame, cmdId("CmdScrollDownPage"));
            sendCommand(frame, cmdId("CmdScrollDownPage"));
            await sleep(1500); // > the 300ms debounce
            ({ state, dump } = await getState(client));
            if (!state.active) {
