...
static void print_help() {
    ...
    fprintf(stderr,
            " -m size or --min=size           Minimum size for anything to be reported (default varies, see below)\n");
...
static bool parse_cmdline(int argc, char* const* argv, DebugFilters& outFilters, std::string& outFile) {
    ...
    static const struct parg_option argsTable[] = {{"name", PARG_REQARG, NULL, 'n'},
    ...
    while ((c = parg_getopt_long(&args, argc, argv, "an:m:f:d:c:F:t:T:h", argsTable, NULL)) != -1) {
        switch (c) {
            case 1:
                ...
                break;
            case 'n':
                outFilters.name = args.optarg;
                break;
            case 'a':
                outFilters.SetMinSize(0);
                break;
            case 'm':
                outFilters.SetMinSize(atof(args.optarg) * 1024);
                break;
            case 'f':
                outFilters.minFunction = atof(args.optarg) * 1024;
                break;
            case 'd':
                outFilters.minData = atof(args.optarg) * 1024;
                break;
            case 'c':
                outFilters.minClass = atof(args.optarg) * 1024;
                break;
            case 'F':
                outFilters.minFile = atof(args.optarg) * 1024;
                break;
            case 't':
                outFilters.minTemplate = atof(args.optarg) * 1024;
                break;
            case 'T':
                outFilters.minTemplateCount = atoi(args.optarg);
                break;
            case '?':
