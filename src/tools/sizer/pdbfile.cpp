...
class IDiaSession : public IUnknown {
  public:
    ...
    virtual HRESULT __stdcall findChildren(IDiaSymbol* parent, enum SymTagEnum symtag, LPCOLESTR name,
                                           DWORD compareFlags, IDiaEnumSymbols** ppResult) = 0;
    virtual HRESULT __stdcall findSymbolByAddr(DWORD isect, DWORD offset, enum SymTagEnum symtag,
    ...
    virtual HRESULT __stdcall findSymbolByRVAEx(DWORD rva, enum SymTagEnum symtag, IDiaSymbol** ppSymbol,
                                                long* displacement) = 0;
    virtual HRESULT __stdcall findSymbolByVAEx(ULONGLONG va, enum SymTagEnum symtag, IDiaSymbol** ppSymbol,
                                               long* displacement) = 0;

    virtual HRESULT __stdcall findFile(IDiaSymbol* pCompiland, LPCOLESTR name, DWORD compareFlags,
                                       IDiaEnumSourceFiles** ppResult) = 0;
    virtual HRESULT __stdcall findFileById(DWORD uniqueId, IDiaSourceFile** ppResult) = 0;

    virtual HRESULT __stdcall findLines(IDiaSymbol* compiland, IDiaSourceFile* file,
                                        IDiaEnumLineNumbers** ppResult) = 0;
    virtual HRESULT __stdcall findLinesByAddr(DWORD seg, DWORD offset, DWORD length,
                                              IDiaEnumLineNumbers** ppResult) = 0;
    ...
    virtual HRESULT __stdcall findLinesByLinenum(IDiaSymbol* compiland, IDiaSourceFile* file, DWORD linenum,
                                                 DWORD column, IDiaEnumLineNumbers** ppResult) = 0;
...
class IDiaSymbol : public IUnknown {
  public:
    ...
    virtual HRESULT __stdcall findChildren(enum SymTagEnum symtag, LPCOLESTR name, DWORD compareFlags,
...
static sChar* BStrToString(BSTR str, const char* defString = "", bool stripWhitespace = false) {
    if (!str) {
        ...
        for (sInt i = 0; i < len; i++) {
            if (stripWhitespace && iswspace(str[i])) continue;
...
void PDBFileReader::ProcessSymbol(IDiaSymbol* symbol, DebugInfo& to) {
    ...
    if (tag == SymTagData) {
        ...
        {
            if (FAILED(type->get_length(&length))) length = 0;
            type->Release();
        } else
            length = 0;
    }
...
void PDBFileReader::ReadEverything(DebugInfo& to) {
    ...
    if (Session->getEnumTables(&enumTables) == S_OK) {
        ...
        if (enumTables->Item(vIndex, &secTable) == S_OK) {
            ...
            while (SUCCEEDED(secTable->Next(1, (IUnknown**)&item, &celt)) && celt == 1) {
                ...
                delete[] objFileStr;
                if (objFileName) SysFreeString(objFileName);
    ...
    */
    ...
    // This is new code that replaces commented out code above. On one not-too-big executable this gets Sizer execution
    // time from 448 seconds down to 1.5 seconds. However it does not list some symbols that are "weird" and are likely
    // due to linker padding or somesuch; I did not dig in. On that particular executable, e.g. 128 kb that is coming
    // from "* Linker *" file is gone.
    ...
    if (SUCCEEDED(Session->get_globalScope(&globalSymbol))) {
        ...
        if (SUCCEEDED(globalSymbol->findChildren(SymTagCompiland, NULL, 0, &enumSymbols))) {
            ...
            if (compilandCount == 0) compilandCount = 1;
...
sBool PDBFileReader::ReadDebugInfo(const sChar* fileName, DebugInfo& to) {
    static const struct DLLDesc {
        const char* Filename;
        IID UseCLSID;
    } DLLs[] = {"msdia71.dll", __uuidof(DiaSource71), "msdia80.dll", __uuidof(DiaSource80), "msdia90.dll",
                __uuidof(DiaSource90), "msdia100.dll", __uuidof(DiaSource100), // VS 2010
    ...
    for (sInt i = 0; DLLs[i].Filename; i++) {
        hr = CoCreateInstance(DLLs[i].UseCLSID, 0, CLSCTX_INPROC_SERVER, __uuidof(IDiaDataSource), (void**)&source);
    ...
    if (source) {
        ...
        fprintf(stderr,
                "  couldn't find (or properly initialize) any DIA dll, copying msdia*.dll to app dir might help.\n");
