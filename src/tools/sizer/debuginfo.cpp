...
sU32 DebugInfo::CountSizeInClass(sInt type) const {
    sU32 size = 0;
    for (sInt i = 0; i < Symbols.size(); i++) size += (Symbols[i].Class == type) ? Symbols[i].Size : 0;
...
sInt DebugInfo::MakeString(const char* s) {
    ...
    if (it != m_IndexByString.end()) return it->second;
...
static bool StripTemplateParams(std::string& str) {
    ...
    while (start != std::string::npos) {
        ...
        if (depth != 0) return isTemplate; // no matching '>', just return
...
void DebugInfo::FinishedReading() {
    ...
    for (sInt i = 0; i < symCount; i++) {
        ...
        if (oldVA != 0) {
            ...
            {
                newVA = oldVA;
                if (newSize >= -adjust) newSize += adjust;
            }
        }
...
sInt DebugInfo::GetFile(sInt fileName) {
    for (sInt i = 0; i < m_Files.size(); i++)
        if (m_Files[i].fileName == fileName) return i;
...
sInt DebugInfo::GetFileByName(sChar* objName) {
    ...
    while ((p = (sChar*)sFindString(objName, "\\"))) objName = p + 1;
...
sInt DebugInfo::GetNameSpace(sInt name) {
    for (sInt i = 0; i < NameSps.size(); i++)
        if (NameSps[i].name == name) return i;
...
sInt DebugInfo::GetNameSpaceByName(sChar* name) {
    ...
    while ((p = (sChar*)sFindString(pp + 2, "::"))) pp = p;

    while ((p = (sChar*)sFindString(pp + 1, "."))) pp = p;

    if (pp != name - 2) {
        sChar buffer[2048];
        sCopyString(buffer, sizeof(buffer), name, 2048 - 1);

        if (pp - name < 2048) buffer[pp - name] = 0;
...
std::string DebugInfo::WriteReport(const DebugFilters& filters) {
    ...
    for (i = 0; i < Symbols.size(); i++) {
        if (Symbols[i].Size < filters.minFunction) break;
        if (Symbols[i].Class == DIC_CODE) {
            ...
            sAppendPrintF(Report, "%5d.%02d: %-80s %-40s %s\n", Symbols[i].Size / 1024,
                          (Symbols[i].Size % 1024) * 100 / 1024, name1, name2, name3);
        }
    }

    // templates
    sAppendPrintF(Report, "\nAggregated templates by size (kilobytes, min %.2f / %i):\n", filters.minTemplate / 1024.0,
                  filters.minTemplateCount);

    std::sort(Templates.begin(), Templates.end(), templateSizeComp);

    for (i = 0; i < Templates.size(); i++) {
        if (Templates[i].size < filters.minTemplate || Templates[i].count < filters.minTemplateCount) break;
        const char* name1 = Templates[i].mangledName.c_str();
        const char* name2 = Templates[i].name.c_str();
        if (filterName && !strstr(name1, filterName) && !strstr(name2, filterName)) continue;
        sAppendPrintF(Report, "%5d.%02d #%5d: %-80s %s\n", Templates[i].size / 1024,
                      (Templates[i].size % 1024) * 100 / 1024, Templates[i].count, name1, name2);
    }

    sAppendPrintF(Report, "\nData by size (kilobytes, min %.2f):\n", filters.minData / 1024.0);
    for (i = 0; i < Symbols.size(); i++) {
        if (Symbols[i].Size < filters.minData) break;
    ...
    */
    ...
    for (i = 0; i < NameSps.size(); i++) {
        if (NameSps[i].codeSize < filters.minClass) break;
        const char* name1 = GetStringPrep(NameSps[i].name);
        if (filterName && !strstr(name1, filterName)) continue;
        sAppendPrintF(Report, "%5d.%02d: %s\n", NameSps[i].codeSize / 1024, (NameSps[i].codeSize % 1024) * 100 / 1024,
                      name1);
    }
    ...
    for (i = 0; i < m_Files.size(); i++) {
        if (m_Files[i].codeSize < filters.minFile) break;
        const char* name1 = GetStringPrep(m_Files[i].fileName);
        if (filterName && !strstr(name1, filterName)) continue;
        sAppendPrintF(Report, "%5d.%02d: %s\n", m_Files[i].codeSize / 1024, (m_Files[i].codeSize % 1024) * 100 / 1024,
                      name1);
    }
