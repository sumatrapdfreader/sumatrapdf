...
IDiaDataSource* LoadDia() {
    if (g_dia_source) return g_dia_source;

    HRESULT hr = E_FAIL;

    // Try creating things "the official way"
    for (int i = 0; msdiaDlls[i].Filename; i++) {
        hr = CoCreateInstance(msdiaDlls[i].UseCLSID, 0, CLSCTX_INPROC_SERVER, __uuidof(IDiaDataSource),
                              (void**)&g_dia_source);
...
void BStrToString(StrBuilder& strInOut, BSTR str, const char* defString, bool stripWhitespace) {
    ...
    for (int i = 0; i < len; i++) {
        c = str[i];
        if (stripWhitespace && isspace(c)) continue;
        if (c < 32 || c >= 128) c = '?';
