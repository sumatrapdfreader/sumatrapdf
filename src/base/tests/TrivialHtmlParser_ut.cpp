...
static void HtmlParser04() {
    ...
    TempStr val = root->GetAttributeTemp(StrL("att"));
...
static void HtmlParser02() {
    ...
    utassert(nullptr == el->next);
    utassert(root == el->up);
    TempStr val = el->GetAttributeTemp(StrL("at1"));
    utassert(str::Eq(val, "<quo&ted>"));
    val = el->GetAttributeTemp(StrL("at2"));
    utassert(str::Eq(val, "also quoted"));
    val = el->GetAttributeTemp(StrL("att3"));
    utassert(str::Eq(val, "notquoted"));
    val = el->GetAttributeTemp(StrL("att4"));
...
static void HtmlParser07() {
    ...
    utassert(1 == p.ElementsCount());
    TempStr val = root->GetAttributeTemp(StrL("umls"));
    utassert(str::Eq(val, "\xC3\xA4\xC3\xB6\xC3\xBC")); // utf8 for äöü
    val = root->GetAttributeTemp(StrL("zerO"));
...
static void HtmlParser09() {
    ...
    TempStr val = root->GetAttributeTemp(StrL("attr"));
...
static void HtmlParser10() {
    ...
    TempStr val = node->GetAttributeTemp(StrL("attr"));
...
static void HtmlParserFile() {
    ...
    el = el->down;
    utassert(el->NameIs(StrL("object")));
    TempStr val = el->GetAttributeTemp(StrL("type"));
