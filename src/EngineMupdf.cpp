...
char* TestXfaResult(const char* pdfPath, int* exitCodeOut) {
    ...
    if (!engine) {
        ...
        if (!epdf || !epdf->pdfdoc) {
                    ...
                    if (valid) {
                        ...
                        fz_try(ctx) {
                            ...
                            if (data_xml) {
                                ...
                                if (serialize_bytes > 0 && BufferContains(ctx, data_xml, "<datasets") &&
                                    (BufferContains(ctx, data_xml, "<firstName>Alice</firstName>") ||
                                     BufferContains(ctx, data_xml, "<topmostSubform") ||
                                     BufferContains(ctx, data_xml, "<xfa:data"))) {
                                    serialize_ok = 1;
                                }
                            }
                        }
            ...
            int fields_with_pa_tpl = 0;
            int fields_bound = 0;
            int fields_with_page_subform = 0;
            int font_families = 0;
            int font_held = 0;
            int font_missing = 0;
            const char* area0 = "";
            ...
            if (xfa && valid) {
                ...
                fz_try(ctx) {
                    ...
                    if (missing_names) {
                        ...
                        if (n >= sizeof(font_missing_names)) {
                            n = sizeof(font_missing_names) - 1;
                        }
                        if (missing_data && n > 0) {
                            memcpy(font_missing_names, missing_data, n);
                        }
                ...
                fields_with_pa_tpl = pdf_xfa_fields_with_pagearea_template(ctx, xfa);
                fields_bound = pdf_xfa_fields_bound(ctx, xfa);
                fields_with_page_subform = pdf_xfa_fields_with_page_subform(ctx, xfa);
                if (page_count > 0) {
...
// Return serialized datasets XML for an XFA document (pdf_xfa_serialize_data).
char* TestXfaSerializeDataResult(const char* pdfPath, int* exitCodeOut) {
    StrBuilder out;
    int exitCode = 1;

    if (!pdfPath) {
        out.AppendFmt("ERROR invalid-args pdf=%s\n", pdfPath ? pdfPath : "(null)");
    } else {
        EngineBase* engine = CreateEngineMupdfFromFile(pdfPath, kindFilePDF, 96, nullptr);
        if (!engine) {
            out.AppendFmt("ERROR engine-create-failed pdf=%s\n", pdfPath);
            ...
            if (!epdf || !epdf->pdfdoc) {
                ...
                if (!xfa || !pdf_xfa_is_valid(ctx, xfa)) {
                    ...
                    fz_catch(ctx) {
                        fz_report_error(ctx);
                        out.AppendFmt("ERROR serialize-failed pdf=%s\n", pdfPath);
                    }
                }
            }
            SafeEngineRelease(&engine);
...
// Select a same-named XFA radio/exclusive group by bounds, save, reload, return field content.
char* TestXfaSelectRadioSaveRoundTripResult(const char* pdfPath, const char* fieldName, int pageNo, float x0, float y0,
                                            float x1, float y1, const char* outPath, int* exitCodeOut) {
    StrBuilder out;
    int exitCode = 1;
    char readBuf[64] = {};

    if (!pdfPath || !fieldName || !fieldName[0] || pageNo < 1 || !outPath || !outPath[0]) {
        out.AppendFmt("ERROR invalid-args pdf=%s field=%s page=%d out=%s\n", pdfPath ? pdfPath : "(null)",
                      fieldName ? fieldName : "(null)", pageNo, outPath ? outPath : "(null)");
    } else {
        EngineBase* engine = CreateEngineMupdfFromFile(pdfPath, kindFilePDF, 96, nullptr);
        if (!engine) {
            out.AppendFmt("ERROR engine-create-failed pdf=%s\n", pdfPath);
