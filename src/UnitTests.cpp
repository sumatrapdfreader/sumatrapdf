...
static void ParseCommandLineTest() {
    {
        auto i = ParseCommandLine(L"SumatraPDF.exe -bench foo.pdf");
    ...
    {
        auto i = ParseCommandLine(L"SumatraPDF.exe -bench foo.pdf -fwdsearch-width 5");
    ...
    {
        auto i = ParseCommandLine(L"SumatraPDF.exe -bench bar.pdf loadonly");
        utassert(2 == i.pathsToBenchmark.size());
    ...
    {
        auto i = ParseCommandLine(L"SumatraPDF.exe -bench bar.pdf 1 -set-color-range 0x123456 #abCDef");
    ...
    {
        auto i = ParseCommandLine(L"SumatraPDF.exe -bench bar.pdf 1-5,3   -bench some.pdf 1,3,8-34");
    ...
    {
        auto i = ParseCommandLine(L"SumatraPDF.exe -presentation -bgcolor 0xaa0c13 foo.pdf -invert-colors bar.pdf");
    ...
    {
        auto i = ParseCommandLine(L"SumatraPDF.exe -bg-color 0xaa0c13 -invertcolors rosanna.pdf");
        utassert(true == i.invertColors);
    ...
    {
        auto i = ParseCommandLine(L"SumatraPDF.exe \"foo \\\" bar \\\\.pdf\" un\\\"quoted.pdf");
        utassert(2 == i.fileNames.size());
    ...
    {
        auto i = ParseCommandLine(L"SumatraPDF.exe -view \"single page\" -zoom 237.45 -scroll -21,-1");
        utassert(0 == i.fileNames.size());
    ...
    {
        auto i = ParseCommandLine(L"SumatraPDF.exe -zoom 35%");
        utassert(0 == i.fileNames.size());
        utassert(i.startZoom == 35.f);
    }

    {
        auto i = ParseCommandLine(L"SumatraPDF.exe -zoom fit-content");
        utassert(i.startZoom == ZOOM_FIT_CONTENT);
        utassert(0 == i.fileNames.size());
    }
}
