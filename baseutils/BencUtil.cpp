...
static void test_parse_torrent(const TCHAR *fileName)
{
    ...
    bobj = benc_obj_from_data(data, (size_t)fileSize);
    assert(bobj);
    if (!bobj)
        goto Exit;

    data2 = benc_obj_to_data(bobj, &lenOut);
...
{
    int i;
    for (i = 0; g_int_data[i].txt != SENTINEL_VAL; i++) {
        ...
        if (valid) {
            ...
            bobjInt = (benc_int64*)bobj;
            expectedVal = g_int_data[i].value;
...
{
    int i;
    for (i = 0; g_str_data[i].txt != SENTINEL_VAL; i++) {
        bool valid;
        benc_obj *bobj;
        ...
        if (txt)
            txtLen = strlen(txt);
        valid = g_str_data[i].valid;
        bobj = benc_obj_from_data(txt, txtLen);
        if (valid) {
            benc_str *bobjStr;
            char *expectedValTxt;
            assert(bobj);
            assert(BOT_STRING== bobj->m_type);
            bobjStr = (benc_str*)bobj;
            expectedValTxt = g_str_data[i].value;
...
static void test_dict_insert()
{
    ...
    for (i = ITERATION_COUNT; i > 0; i--) {
        ...
        assert(ok);
        dictLen = benc_dict_len(dict);
        assert(ITERATION_COUNT + 1 - i == dictLen);
        found = benc_dict_find(dict, key, keyLen);
        assert(found == obj);
    }
    benc_obj_delete((benc_obj*)dict);
...
void u_benc_all(void)
{
    ...
    test_parse_array();
    test_parse_dict();
    // test_parse_torrent(_T("torrent1.torrent"));
}
#endif
