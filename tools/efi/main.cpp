...
const char* g_symTypeNames[SymTagMax] = {"Null",
                                         "Exe",
                                         "Compiland",
                                         "CompilandDetails",
                                         "CompilandEnv",
...
const char* g_thunkTypeNames[] = {"NoType", "Adjustor", "VCall", "PCode", "Load", "TrampInc", "TrampBranch"};

const char* GetThunkTypeName(DWORD thunkType) {
    if (thunkType >= dimof(g_thunkTypeNames)) return "Unknown";
...
int StringInterner::Intern(const char* s, bool* alreadyPresent) {
    ...
    Str internedString;
    bool inserted = strToInt.Insert(Str(s), idx, &idx, &internedString);
    if (!inserted) {
        if (alreadyPresent) {
            *alreadyPresent = true;
        }
    ...
    intToStr.Append(internedString.s);
    if (alreadyPresent) {
...
static const char* GetSymTypeName(int i) {
    if (i >= SymTagMax) return "<unknown type>";
    if (g_compact)
...
static const char* GetSectionType(IDiaSectionContrib* item) {
    ...
    if (code && !initData && !uninitData) return "C";
    if (!code && initData && !uninitData) return "D";
    if (!code && !initData && uninitData) return "B";
    return "U";
}

static const char* GetUdtType(IDiaSymbol* symbol) {
    DWORD kind = (DWORD)-1;
    if (FAILED(symbol->get_udtKind(&kind))) return "<unknown udt kind>";
    if (UdtStruct == kind) return "struct";
    if (UdtClass == kind) return "class";
    if (UdtUnion == kind) return "union";
...
#else
    DWORD undecorateOptions = UNDNAME_NO_MS_KEYWORDS | UNDNAME_NO_FUNCTION_RETURNS | UNDNAME_NO_ALLOCATION_MODEL |
                              UNDNAME_NO_ALLOCATION_LANGUAGE | UNDNAME_NO_THISTYPE | UNDNAME_NO_ACCESS_SPECIFIERS |
                              UNDNAME_NO_THROW_SIGNATURES | UNDNAME_NO_MEMBER_TYPE | UNDNAME_NO_RETURN_UDT_MODEL |
                              UNDNAME_NO_ECSU;
#endif
...
static void DumpType(IDiaSymbol* symbol, int deep) {
    ...
    if (symbol->get_symTag(&symtag) != S_OK) return;
    ...
    if (SymTagData == symtag) {
        if (symbol->get_locationType(&locType) != S_OK) return; // must be a symbol in optimized code
...
static void DumpSymbol(IDiaSymbol* symbol) {
    ...
    if (SymTagData == tag) {
        ...
        if (symbol->get_type(&type) == S_OK) // no SUCCEEDED test as may return S_FALSE!
        {
            ...
            if (s && *s) dataTypeName = str::Dup(s);
            type->Release();
        }
    }
    if (SymTagThunk == tag) {
        DWORD dwThunkKind;
        if (S_OK == symbol->get_thunkOrdinal(&dwThunkKind)) thunkTypeName = GetThunkTypeName(dwThunkKind);
    } else {
        symbol->get_length(&length);
    }
    ...
    if (SymTagData == tag) {
        ...
        g_report.AppendFmt("%s|%d|%d|%d|%d|%s|%s\n", typeName, (int)section, (int)length, (int)offset, (int)rva,
                           nameStr, tmp);
        free(dataTypeName);
...
static void DumpSymbols(IDiaSession* session) {
    ...
    for (;;) {
        ...
        if (FAILED(hr) || (numFetched != 1)) break;
    }
...
static void ProcessPdbFile(const char* fileNameA) {
    ...
    if (g_dumpTypes) DumpTypes(session);

    if (g_dumpSections) DumpSections(session);

    if (g_dumpSymbols) DumpSymbols(session);
...
static bool ParseCommandLine(int argc, char** argv) {
    ...
    if (!g_fileName) goto InvalidCmdLine;
