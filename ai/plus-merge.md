...
| 2026-07-11 | I. Home page list view | **cherry-picked** | master independently grew its own list view (`32d638e29`, #4909, richer rows: thumbnail/size/pin/remove). Cherry-picked from the fork: the directory-path column (right-aligned, muted, path-ellipsized, only when the name leaves room) and the `HomePageViewMode = thumbnails\|list` setting shape (replaces the prerelease-only `HomePageShowList` bool). Also fixed the fixed-64px size column truncating values like "177.80 KB" (now sized to its text). Skipped: their shell file-type icons (we show real thumbnails), tips/promos banner (`ShowTips`), dark-theme thumbnail bookkeeping (tied to their themes-recolor-pages design). |
