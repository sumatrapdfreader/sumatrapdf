...
def EngineDump(EngineDumpExe, file, tgaPath):
    proc = Popen([EngineDumpExe, file, "-full", "-render", tgaPath],
                 stdout=PIPE)
...
def TgaCmpColor(col1, col2):
    # returns 0 for identical colors, 1 for similar colors and 2 for different
    # colors
    if col1 == col2:
        return 0
...
def BitmapDiff(tgaRef, tgaCmp, tgaDiff):
    ...
    diff = [struct.pack(
        "<BBBHHBHHHHBB", 0, 1, 9, 0, 3, 24, 0, 0, width, height, 8, 0),
        "\x00\x00\x00", "\x00\x00\x80", "\x00\x00\xFF"]
    for i in range(height):
        ...
        while j < width:
            color = TgaCmpColor(
                refLine[j * 3:j * 3 + 3], cmpLine[j * 3:j * 3 + 3])
            k = j + 1
...
def RefTestFile(EngineDumpExe, file, refdir):
    ...
    for file in fnmatch.filter(os.listdir(refdir), base + "-[0-9]*.ref.tga"):
        ...
        tgaCmpPath, tgaDiffPath = tgaRefPath[:-8] + \
            ".cmp.tga", tgaRefPath[:-8] + ".diff.tga"
...
def main(args):
    ...
    if len(args) > 1 and args[1].lower().endswith(".exe"):
        ...
        EngineDumpExe = pjoin(os.path.dirname(__file__),
                              "..", "obj-dbg", "EngineDump.exe")
    ...
    # collect all directories to test (and the corresonding reference
    # directories)
    dirs, ix = [], 1
    while ix < len(args):
