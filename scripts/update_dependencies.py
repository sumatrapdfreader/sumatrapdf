...
import util2
import update_vs
import update_vs2008
pjoin = os.path.join

DIRS = ["src", pjoin("src", "utils"), pjoin("src", "installer"), pjoin("src", "ifilter"), pjoin("src", "browserplugin"), pjoin(
    "src", "previewer"), pjoin("src", "mui"), pjoin("src", "memtrace"), pjoin("src", "regress"), pjoin("src", "uia")]
INCLUDE_DIRS = DIRS + [pjoin("mupdf", "include")]
OBJECT_DIRS = {"src\\utils": "$(OU)", "src\\browserplugin": "$(ODLL)", "src\\ifilter": "$(ODLL)", "src\\previewer":
               "$(ODLL)", "src\\mui": "$(OMUI)", "src\\memtrace": "$(OM)", "src\\uia": "$(OUIA)"}  # default: "$(OS)"
...
def prependPath(files, basefile=None):
    ...
    for file in files:
        if file in ["string.h", "math.h"]:
            # skip names of system headers which also exist in
            # mupdf/include/mupdf/fitz
            continue
...
def flattenDependencyList(dependencies):
    ...
    for file in dependencies.keys():
        if dependencies[file]:
            ...
            # TODO: normalizing paths already in prependPath makes
            # getObjectPath fail under cygwin
            deplist = sorted(
                dependencies[file], key=lambda s: str.lower(s.replace("/", "\\")))
            for depgroup in util2.group(deplist, DEPENDENCIES_PER_LINE):
                flatlist.append("%s\\%s.obj: $B\\%s" %
                                (opath, filename, " $B\\".join(depgroup)))
    return flatlist
