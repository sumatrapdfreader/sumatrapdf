...
DIR_STRUCT = [
    ...
    (pjoin("src", "browserplugin"),
     ("Integration DLLs", "Browser Plugin")),
    ...
    (pjoin("mupdf", "include", "mupdf", "fitz"),
     ("mupdf", "include", "fitz")),
    (pjoin("mupdf", "include", "mupdf", "pdf"),
     ("mupdf", "include", "pdf")),
...
def parseNodes(root):
    ...
    for node in getSiblingNodes(root.content):
        assert node.name == "File" and node.getAttr(
            "RelativePath") or node.name == "Filter" and node.getAttr("Name")
...
def findAddedFiles(knownPaths):
    ...
    for dir in DIR_STRUCT:
        ...
        added += [(path, dir[1])
                  for path in all_files if not path in knownPaths and isRelevant(path)]
    return added
