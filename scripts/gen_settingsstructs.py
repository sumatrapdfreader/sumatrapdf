...
class Field(object):
    ...
    def __init__(self, name, type, default, comment, internal=False, expert=False, doc=None, version=None, prerelease=False):
        ...
        self.cname = name[
            0].lower() + name[1:] if name else None
        # "expert" prefs are the ones not exposed by the UI
        self.expert = expert
        self.docComment = doc or comment
        # version in which this setting was introduced
        self.version = version or "2.3"
    ...
    def cdefault(self, built):
        ...
        if self.type is Float:
            ...
            return '(intptr_t)"%g"' % self.default
        if self.type is Int:
...
class Struct(Field):
    ...
    def __init__(self, name, fields, comment, structName=None, compact=False, internal=False, expert=False, doc=None, version=None, prerelease=False):
        ...
        super(
            Struct, self).__init__(name, Type("Struct", "%s" % self.structName),
                                   fields, comment, internal, expert, doc, version, prerelease)
        if prerelease:
            self.type.name = "Prerelease"
        elif compact:
...
class Array(Field):
    ...
    def __init__(self, name, fields, comment, structName=None, internal=False, expert=False, version=None):
        ...
        super(Array, self).__init__(name, Type("Array", "Vec<%s *> *" %
                                               self.structName), fields, comment, internal, expert, None, version)
...
class CompactArray(Field):
    ...
    def __init__(self, name, type, default, comment, internal=False, expert=False, doc=None, version=None):
        super(
            CompactArray, self).__init__(
            name, Type("%sArray" % type.name, "Vec<%s> *" %
                       type.ctype), default, comment, internal, expert, doc, version)


class Comment(Field):

    def __init__(self, comment, expert=False, version=None):
        super(Comment, self).__init__("", Type("Comment", None),
                                      None, comment, False, expert)
...
PrinterDefaults = [
    Field("PrintScale", Utf8String, "shrink",
          "default value for scaling (shrink, fit, none)"),
    Field("PrintAsImage", Bool, False,
...
FixedPageUI = [
    ...
    CompactArray(
        "GradientColors", Color, None,  # "#2828aa #28aa28 #aa2828",
...
EbookUI = [
    # default serif font, a different font is used for monospaced text
    # (currently always "Courier New")
    Field("FontName", String, "Georgia",
          "name of the font. takes effect after re-opening the document"),
    Field("FontSize", Float, 12.5,
          "size of the font. takes effect after re-opening the document"),
    Field("TextColor", Color, RGB(0x5F, 0x4B, 0x32), "color for text"),
    Field("BackgroundColor", Color, RGB(0xFB, 0xF0, 0xD9),
...
FileSettings = [
    ...
    Field("UseDefaultState", Bool, False,
          ...
          "the values below)"),
    # NOTE: fields below UseDefaultState aren't serialized if
    # UseDefaultState is true!
...
rememberedDisplayState = [
    "DisplayMode", "ScrollPos", "PageNo", "Zoom", "Rotation",
    "WindowState", "WindowPos", "ShowToc", "SidebarDx", "DisplayR2L", "ReparseIdx", "TocState"]

GlobalPrefs = [
    Comment(
        "For documentation, see http://blog.kowalczyk.info/software/sumatrapdf/settings%s.html" %
        util2.get_sumatrapdf_version()),
    EmptyLine(),
    ...
    CompactArray(
        "ZoomLevels", Float, "8.33 12.5 18 25 33.33 50 66.67 75 100 125 150 200 300 400 600 800 1000 1200 1600 2000 2400 3200 4800 6400",
    ...
    Field(
        "DefaultDisplayModeEnum", Type(
            None, "DisplayMode"), "DM_AUTOMATIC",
...
def BuildMetaData(struct, built=[]):
    lines, data, names = [], [], []
    fullName = struct.structName + \
        ("" if struct.structName not in built else "_%d_" %
         built.count(struct.structName))
    for field in struct.default:
        if field.internal:
            continue
        data.append(
            ("offsetof(%s, %s)" % (struct.structName, field.cname), "Type_%s" %
             field.type.name, field.cdefault(built)))
    ...
    lines += ["\t{ %s }," %
              line for line in FormatArrayLine(data, "%s, %s, %s")]
    lines.append("};")
    # gFileStateInfo isn't const so that the number of fields can be changed
    # at runtime (cf. UseDefaultState)
    lines.append("static %sStructInfo g%sInfo = { sizeof(%s), %d, g%sFields, \"%s\" };" % (
        "const " if fullName != "FileState" else "", fullName, struct.structName, len(names), fullName, "\\0".join(names)))
    return "\n".join(lines)
