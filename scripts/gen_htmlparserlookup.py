...
def createFastFinder(list, type, default, caseInsensitive, funcName=None):
    ...
    while list:
        ...
        if len(name) < 4:
            # no further comparison is needed for names less than 4 characters
            # in length
            output.append('case CS%d(%s): return %s;' %
                          (len(name), splitChars(name), value))
        else:
            ...
            while True:
                if len(name) == 4:
                    ...
                    rest = name[4:]
                    output.append(
                        '	if (%d == len && CS%d(%s) == STR%di(name + 4)) return %s;' %
                        (len(name), len(rest), splitChars(rest), len(rest), value))
                else:
                    output.append(
                        '	if (%d == len && str::EqNI(name + 4, "%s", %d)) return %s;' %
    ...
    output = Template_Find_Function % (
        type, funcName or type, "\n	".join(output), default)
    if not caseInsensitive:
        ...
        assert not [c for c in output if c >
                    '\x7f'], "lower() only supports ASCII letters"
    return unTab(output)
...
def createFastSelector(fullList, nameList, funcName, type):
    cases = ["case %s:" %
             value for (name, value) in fullList if name in nameList]
...
List_Other_Tags = "image mbp:pagebreak pagebreak subtitle svg"
...
# these tags must all also appear in List_HTML_Tags or List_Other_Tags
...
def main():
    ...
    tags = [(name, getEnumName(name, "Tag"))
            for name in sorted(List_HTML_Tags.split() + List_Other_Tags.split())]
    attrs = [(name, getEnumName(name, "Attr"))
    ...
    code_selfclosing = createFastSelector(
        tags, List_Self_Closing_Tags.split(), "IsTagSelfClosing", "HtmlTag")
    code_inlinetag = createFastSelector(
        tags, List_Inline_Tags.split(), "IsInlineTag", "HtmlTag")
    code_alignattr = createFastFinder(
        aligns, "AlignAttr", "Align_NotFound", True)
    code_htmlentity = Template_Entities_Comment + "\n" + \
        createFastFinder(List_HTML_Entities, "uint32_t",
                         "(uint32_t)-1", False, "HtmlEntityRune")
    code_cssprop = createFastFinder(cssProps, "CssProp", "Css_Unknown", True)
    code_csscolor = createFastFinder(
        cssColors, "ARGB", "MKRGBA(0,0,0,0)", True, "CssColor")
