...
def runBenchmark(SumatraPDFExe, file, repeats):
    ...
    proc = Popen([SumatraPDFExe] + ["-bench", file]
                 * repeats, stdout=PIPE, stderr=PIPE)
...
def main():
    if not sys.argv[1:]:
        log("Usage: %s [<SumatraPDF.exe>] <file1.pdf> [<file2.pdf> ...]" %
    ...
    if sys.argv[1].lower().endswith(".exe"):
        ...
        SumatraPDFExe = os.path.join(
            os.path.dirname(__file__), "..", "obj-rel", "SumatraPDF.exe")
