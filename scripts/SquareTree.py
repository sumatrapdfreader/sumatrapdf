...
class Node(object):
    ...
    def GetChild(self, name, idx=0):
        children = [item for item in self.data if type(
            item[1]) is Node and item[0].lower() == name.lower()]
        return (children + [(name, None)] * (idx + 1))[idx][1]

    def GetValue(self, name, idx=0):
        values = [item for item in self.data if type(
            item[1]) is not Node and item[0].lower() == name.lower()]
...
def Parse(data, level=0):
    ...
    while data:
        ...
        if sep == "[" and (not value or value[0] in ["#", ";"]) or nodeKey:
            ...
            # close the subnode (or ignore it if we're already at the top
            # level)
            if level > 0:
                return node, data[1:]
            data = data[1:]
        elif not key and sep == "[" and value.endswith("]"):
            # interpret INI style section headers as the names of top-level
            # nodes
            if level > 0:
                return node, data
