...
- Touches: `EngineMupdf.cpp` (+2,800 lines), `RenderCache.cpp`, `libsumatrapdf.def`
...
5. **Port approved items:**
   ...
   - Build (`bun cmd/build.ts -dbg`) and run unit tests (`bun cmd/run-unit-tests.ts -dbg`).
...
- Any fix in group K may already exist in master in a different form — verify before
  porting.
- Their `libsumatrapdf.def` / premake file lists differ; regenerate ours rather than merging.
...
| 2026-07-28 | K: remaining multi-monitor DPI (`30a67f0fa` + rest of `1369facda`) | **merged** | for #5827: `GetApp*ForDpi`, `HwndSetTreeFontForDpi` (TVM_SETITEMHEIGHT), trust `WM_DPICHANGED` wParam via `frameDpi`, full chrome refresh (toolbar/rebar/find bar/sidebar/caption), ENTERSIZEMOVE defer + settle, maximized NCCALCSIZE work-area clip; no TreeView HWND recreate (VBox ownership; item-height fix covers metrics) |
