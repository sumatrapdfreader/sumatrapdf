...
- add `RememberViewOffsetOnPageTurn` advanced setting (default **false**): next/previous page keeps the same view position on the new page instead of jumping to the top, so zoomed-in pages of similar size stay lined up (fixes #5069)
...
- new **Change eBook Settings** command (`CmdChangeEbookSettings`): a dialog with the font, size and line spacing for a reflowable document, which shows the CSS it generates from them and lets you take over and write that CSS yourself. Applies to the open document only or to all ebooks (fixes #4600)
