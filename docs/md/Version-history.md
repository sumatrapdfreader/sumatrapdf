...
- Annotations window: the list is as tall as the annotations (up to 12 rows) and keeps that height while you move between them. Rect, Author and Date are always shown in a two-column table (empty when the annotation has no value). Per-annotation options (alignment, font, color, and so on) sit in the same kind of table. Contents is a label above a 6-line edit box. Each list row shows the type and contents on the left and the page number on the right
