...
- The **File** menu no longer lists **Convert PDF to Images...**, **Apply Redactions**, **Insert Image...** and **Sign Document...**; they remain in the right-click **Document** submenu and the command palette
- `ShowChaptersInEbooks` (off by default) shows where you are in a chaptered ebook as a chapter and a page within it: the toolbar, Go to Page, and the page-info tip. Off, those show one page number for the whole book. The saved position stays a chapter bookmark either way, and Next / Previous Page still cross chapters. Opening lays out only the chapter you were reading (the first chapter, the first time) and counts the remaining chapters in the background; the page total updates when that finishes. A debug build shows that progress at the bottom left
- The in-app manual (F1) has a theme switch in its top-right corner: light, dark, or follow the app's theme (the default), remembered in the `HelpTheme` setting. Its `Ctrl + K` search gained an **Ask a question** mode that sends the question to Grok, ChatGPT or Claude in your browser, with the SumatraPDF documentation as context, like the search on the website
- Home page thumbnails use a cover image stored next to the document with the same base name (`Book.jpg`, `.jpeg` or `.png` beside `Book.epub`), the way Calibre saves covers, or else the cover an EPUB declares in its metadata, instead of rendering page 1 (fixes #6236)
- **Navigate Files in Folder** has Back, Forward, Up and Home buttons before the folder name. Back / Forward (`Alt + Left` / `Alt + Right`) walk the folders visited in this session (kept across re-opening the window), Up (`Alt + Up`, `Backspace`, or the `..` entry) goes to the parent (from a drive root: to Home), Home lists all drives followed by Explorer's Quick access folders and files (fetched once; F5 there re-reads them). A search field under the folder name filters the list as you type, matching like the command palette (every word, case and accent insensitive); `Ctrl + F` or just typing while the list has focus goes to it, Up/Down move the selection from it, Esc clears it, and changing folder clears it. Clicking the folder path edits it in place: Enter goes to that directory (a file path opens its folder with the file selected, quotes from Explorer's Copy as path are fine), Esc or a path that doesn't exist restores the previous one (#6232)
...
- **Go To Home Page** (`CmdGoToHomePage`, command palette) switches to the Home tab. With `NoHomeTab = true` there is none, so it adds one as the first tab; close it with its ✕ (fixes #6228)
...
- **Print Selection...** (`CmdPrintSelection`, selection context menu, command palette): with a rectangular (Ctrl+drag) selection, opens the print dialog with **Selection** already chosen so only that area prints. Printing a selection was always possible via the dialog's Selection radio button; this makes it discoverable (fixes #6222)
...
- Command Palette **Toggle Boolean Setting** lists the advanced settings (`=` prefix) with their current values, non-default ones in bold and listed first. Enter on a `true` / `false` setting toggles it and closes. Any other setting asks for a value: the query becomes `=<name> = <value>`, where a setting restricted to a fixed set (`Toolbar`, `Scrollbars`, `PrintScale`, …) lists them to pick from and anything else takes what you type, starting from the current value. The change takes effect immediately, no restart. The whole thing can be typed at once (`=ZoomIncrement = 25`), and the name can be the last part of a dotted setting (`Units` for `FixedPageUI.PageGrid.Units`) when only one setting ends with it. The selected setting's description is shown under the list, as in Advanced Settings. Compact settings like `WindowMargin` stay Advanced Settings only, lists stay settings-file only
- Command Palette `&` opens the current document's thumbnail grid with the current page selected
- Closing the end-of-document "open next file" hint stops showing it; `ShowFileNavigateHint` turns it back on (fixes #6188)
- `ScrollEdgeTurnsPage` (on by default) controls whether scrolling past the top or bottom of a zoomed-in page in single page / facing / book view goes to the previous / next page. Turn it off to stay on the page and change pages only with the keyboard, toolbar or scrollbar. A page that fits the window has nothing to scroll, so a wheel notch still turns it (fixes #6201)
- the Windows 11 print dialog's **More settings** pane now offers the printer's own settings - color vs black and white, one- vs two-sided, collation, paper size and type, quality, pages per sheet, input tray, binding, stapling, hole punch - instead of only the input tray. Settings the selected printer doesn't support are left out. Unlike the classic print dialog, this one has no button that opens the driver's property sheet, so anything not offered here couldn't be reached at all (discussion #6202)
- `PrinterUI` chooses which dialog **Print** (`Ctrl + P`) opens: empty or `auto` (the Windows 11 dialog with print preview where it's available, the classic one everywhere else), `modern` (the same, said explicitly), or `classic` (always the classic dialog). Only the classic dialog has a **Preferences** button, which opens the printer driver's own property sheet; the Windows 11 dialog has no way to show it, so set `classic` to reach a driver setting that its **More settings** pane doesn't offer. `modern` still falls back to the classic dialog when Windows can't show the modern one - before Windows 11, when running elevated, or when printing a selection (discussion #6202)
- Menu and command names say settings, not options: **Settings → Options...** is now **Settings...** and its dialog is titled **Settings** (was **SumatraPDF Options**); **Advanced Options...** is now **Advanced Settings...**. `CmdAdvancedOptions` is gone, the menu item runs `CmdAdvancedSettings`, the dialog; a shortcut definition naming `CmdAdvancedOptions` still works. **Settings → Open Advanced Settings File...** opens the settings file in the .txt editor (`CmdOpenSettingsFile`)
- The command palette can list a command under more than one name; **Navigate Files in Folder...** is also **Browse Files In Folder...**, and **Advanced Settings...** is also **Advanced Options...**
- **Toggle Free Pan** (`CmdToggleFreePan`, command palette): the view can go past the page edges by half a window, so any part of a page, a corner of a technical drawing included, can be brought to the center of the screen instead of being stuck at the window edge. Fit zooms and page placement are unchanged; it only adds scroll room. Remembered per document (discussions #6224, #6164)
- Starting a search adds a navigation point at the view it started from, once per search, so `Backspace` / `Alt + Left` returns there after find-as-you-type and Find Next moved through matches. The session-only `/` favorite marks the same view (fixes #6230)
- the Windows 11 print dialog's **More settings** pane has a **Page scaling** dropdown with the classic Advanced tab's choices: shrink to printable area, fit to printable area, stretch to fill paper, actual size. It starts at `PrinterDefaults.PrintScale`
- `ShowPageNumberInTabs` (off by default) shows the current page as `n/N` after the file name on tabs; the name ellipsizes first
- the home page shows reading progress (`12/62`, or `3:5/20` for chaptered ebooks) on list rows and as a badge on thumbnails, from the last place you were in that file. Turn it off with `ShowHomePageReadingProgress = false` (fixes #6199)
- PDF documents have an **Edit PDF** toggle at the end of the standard toolbar. It opens a second row of SVG annotation tools for highlight, underline, squiggly, strike out, text, free text, lines, shapes, ink, redaction, stamps, carets, and file attachments; text-markup tools are enabled only while text is selected. The annotation toolbar ends with **Find Annotation**, before the save buttons, which opens a floating **Annotations** window: a search box (`filter N annotations`) over the list of the document's annotations (arrows, Page Up/Down, Home/End). The list caret moves immediately; selecting the annotation on the page is debounced. Esc clears the search, or returns focus to the page if the box is already empty. A click in the list selects, a double-click selects and jumps to it. **Find Annotation** again, or the window's close button, hides it (fixes #6086). The window has **Delete Annotation**, **Discard changes**, **Save changes to** the current file, and **Save changes to a new PDF** at the bottom; save and discard stay disabled until there are unsaved changes, and delete stays disabled until an annotation is selected. Shift+click and Ctrl+click select multiple annotations; the delete button then reads **Delete N annotations**. Del deletes the selection. Choosing **Text**, **Free Text**, **Stamp**, **Caret**, **File Attachment**, **Line**, **Polyline**, **Square**, **Circle**, or **Ink** from this toolbar or the Command Palette lets you position the annotation on the page before placing it. A free text, stamp, caret, or file attachment follows the cursor as a preview until you click the page; free text previews as the text box it will create, sized to its text. Drag out a rectangle or ellipse, or click its two opposite corners; hold **Shift** for a square or circle. Ink commits each stroke when you release; the tool stays on for another. **Esc** or closing the hint leaves the tool. Finish a polyline with a double-click, right-click, **Space**, or **Enter**. Clicking an existing annotation in Edit PDF mode selects it and shows a compact property row under it (color swatch, opacity, border, font, icon, line endings, Contents, and **Delete Annotation** at the end). A file attachment's row starts with **Attach File**, and **Save Attachment** once a file is embedded. Icon, line-ending, and text-alignment menus show the glyph next to the name. Resizing a free text annotation drags only its outline; the annotation itself is rewritten once, when the drag ends, because re-laying out its text on every mouse move is slow. Outside Edit PDF mode a click does not select an annotation (including free text and stamps). `Ctrl + click` an annotation to enter Edit PDF mode (the annotation toolbar appears and the **Edit PDF** button is checked) and select it. The hover tip ("Highlight annotation. Ctrl+click to edit.") is no longer shown. `Ctrl + A` and `Ctrl + V` in a text box are that box's select-all and paste, not the page's - the annotation text editors, the page-number box, form fields (`Ctrl + C`, `Ctrl + X` and `Ctrl + Z` already were). A free text annotation's property row leads with the colour of its text, and its chips name what they change: **Text Color**, **Background Color**, **Border Width**, **Edit text**. Other annotation types get **Border Width** and **Edit Note** too. A free text annotation is edited where it sits on the page: double-click it, or press **Contents** on its property row, and an edit box opens over it in the annotation's font and size on a white background. Placing a new one opens the box straight away. The box grows with what you type and is sized the way MuPDF will lay the text out, so nothing wraps that the box showed whole. **Enter** makes a new line, and **Ctrl + Enter**, **Contents** again, clicking away or **Esc** goes back to the rendered annotation. Making a highlight, underline, squiggly or strike-out from a text selection now consumes the selection: the selection and its floating toolbar go away and the new annotation is selected instead, like every other annotation. Leaving Edit PDF mode leaves no annotation-editing UI behind: the selected annotation is deselected (its marker and resize handles go with it) and a half-finished placement is cancelled. The page context menu gathers everything annotation-related (create from selection, create under cursor, cut / copy / paste / delete, save, apply redactions) under one **Annotations** submenu, which ends with **Apply Redactions**, **Save changes**, **Save to new file** and **Discard changes**; each is disabled when there is nothing to act on. Copy and Paste Annotation show the `Ctrl + C` / `Ctrl + V` keys that reach them. Creating an annotation selects it. `CmdCreateAnnot*` turns Edit PDF mode on only with the `openedit` argument (`Shift + A` / `Shift + U`). Pasting an annotation still turns Edit PDF mode on. `openedit` still opens the Contents editor. The standalone Annotations editor window and the context-menu **Edit Annotations** command are gone. The command is `CmdToggleEditPDF` ("Toggle Edit PDF"); `CmdTogglePdfAnnotationsToolbar` still works in shortcuts and `ToolbarCustomLayout`
- Every Edit PDF toolbar button that creates a colored annotation (highlight, underline, squiggly, strike out, text, free text, line, polyline, square, circle, polygon, ink, stamp, caret, file attachment) and every color chip of a selected annotation share one color drop-down: the preset colors (`Annotations.PresetColors`) as swatches, plus a button that opens the color dialog on the whole set, where a color can be given an opacity. That dialog is owned by the main window, so it no longer opens behind it. A color carries the annotation's opacity (issue #6063), so an annotation with a color chip has no separate **Opacity** chip and its chips show the colors as they look on the page. Annotations with two colors, like a square or a circle, get a drop-down for each, and their interior can be left out with the drop-down's first swatch. Picking a color on a toolbar button is the color the next annotation of that type is made in, kept in a setting of its own (`Annotations.LineColor`, `SquareColor`, `InkColor` and so on; the text button uses `TextIconColor` and free text `FreeTextColor`). **Redact** has no such drop-down: its color is the box that covers the text. The **Ink** button's drop-down has more than colors: a preview of the stroke it will lay down and a **Thickness** slider under it, from Thin to Thick, which is `Annotations.InkBorderWidth` in points (6 by default). The preview follows the slider as you drag it, and so does the stroke you draw on the page. A selected ink annotation's color chip opens that same drop-down, where the slider is that stroke's own width, applied when you let go; it therefore has no **Border Width** chip. The default colors for underline, squiggly and strike out are now the nearest preset colors, and the first preset color is the yellow that highlights have always used
- A free text annotation's property row has **Bold**, **Italic** and **Underline** toggles next to **Font**. The **Font** menu lists Courier, Helvetica and Times Roman plus **Other Font (Embedded)...**, which picks any installed font with the Windows font dialog. Such a font is embedded in the PDF, so it looks the same everywhere; as that can add hundreds of kilobytes or more, picking one asks first. The font and style are written as the annotation's `/DS` default style, which Acrobat and other editors read too (fixes #6198)
- **Redact** marks selected text, or a dragged rectangle, for removal. The marks are red outlines and can still be moved or deleted. **Apply Redactions** permanently deletes the marked content from the file, after a confirmation. Save a copy afterwards; once the file is saved this cannot be undone
- The Edit PDF toolbar has a **Highlighter** next to **Highlight**: a marker pen you drag across the page, painting a wide translucent stroke over pictures and blank areas as well as text, with no text selection to make first. Release the mouse or pen to finish (`Enter` still works); **Esc** cancels. Closing the placement hint at the bottom of the window also finishes ink and polyline, so a touch device does not need a keyboard. The stroke keeps a constant on-screen width whatever the zoom and is saved as an ink annotation in `HighlightColor` at 40% opacity, so it can be moved, recolored or deleted like any other annotation
...
- Polygon, polyline, and ink annotations can be moved by dragging them, like other shape annotations. Rich media and 3D annotations can be moved and resized too: the PDF spec and MuPDF both treat their `/Rect` as theirs to change
...
- Picking an action on the floating selection toolbar hides it until the selection changes. Markup actions (highlight, underline, …) turn the selection into the annotation; **Copy to clipboard** keeps the selection and briefly shows "Copied to clipboard" in the bottom-left corner
...
- **Undo** (`Ctrl + Z`) and **Redo** (`Ctrl + Shift + Z`) for changes to a PDF, with buttons at the end of the Edit PDF toolbar that are greyed out when there is nothing to step to. Built on MuPDF's journal, so it covers annotations, form fields and applied redactions; one gesture is one step (creating, pasting or resizing an annotation comes back in a single undo), and undoing every change marks the document unmodified again
- The Edit PDF toolbar ends with two save buttons: one writes the changes back to the file you are reading (its tooltip names it), the other saves them into a new PDF. Both are greyed out while there is nothing to save
...
- On the home page, hovering the **SumatraPDF** title shows the About box below it as a borderless dropdown (no second logo). Its links are clickable, and **Copy program and machine info to clipboard** is inside the box. Moving the mouse off the title hides it
- The home page has a command-palette button to the left of the **SumatraPDF** title and a **?** keyboard-shortcuts button to the right of it. The Keyboard Shortcuts window can be moved by dragging it
- Advanced Settings can edit compact values such as `FixedPageUI.PageSpacing` and `ComicBookUI.PageSpacing` (written as `4 4`). Changing them applies to open documents without restarting. Comics and image collections use `ComicBookUI.PageSpacing`, not `FixedPageUI.PageSpacing`
- The Read Aloud playback bar uses a compact speed slider (circle on a bar, with the rate next to it) instead of a click-to-cycle button. Speeds go from 0.5x to 3.5x in 0.25 steps from 2x up (2, 2.25, 2.5, 2.75, 3, 3.25, 3.5)
- The **Annotations** list (Find Annotation) opens beside the main window on the side with more room. When the window is maximized or fullscreen it sits on the right edge of the screen, vertically centered. Moving it is remembered until Sumatra is closed, not saved in settings
- **Help: Manual** (F1) opens next to the main window, in the upper half, on the side with more room, wide enough to show the table-of-contents sidebar. Its position and size are remembered across sessions and kept fully on-screen after a resolution or monitor change (fixes #6013)
...
- Command Palette: type `%` for the table of contents of the current document (like `$` for favorites). `% TOC` appears in the mode row when the document has a TOC
- Command Palette: type `*` to search and jump to annotations in the current PDF (same rows and filter syntax as **Find Annotation**). `* Annotations` appears in the mode row after annotations have been collected and the document has at least one. Collection runs once in the background and is shared with **Find Annotation**, including when there are none
...
- **Page Grid** (`CmdTogglePageGrid`, View menu) overlays graph paper on PDF, EPUB, and other paginated document pages, like PDF-XChange’s Show Grid (not comics). Showing the grid is session-only. **Configure Page Grid** (`CmdConfigurePageGrid`) sets units, spacing, subdivisions, origin, color and line style (dots, dotted lines, or solid); those values are saved in `FixedPageUI.PageGrid` (default 1 inch major / 4 subdivisions, color 128,128,255, dots), and can be restored with **Reset to defaults** (fixes #4398)
...
- `SaveMemory` (0–100, default 50) controls how aggressively unused page and image caches are freed: 0 keeps them until an allocation fails, 100 drops them as soon as a page is off-screen
...
- rename `CmdDiscardAnnotations` to `CmdDiscardChanges` ("Discard Changes"): reloads the current document from disk, dropping unsaved annotations and form changes; available from the tab context menu when there are unsaved changes and with `Ctrl + K`, `Discard Changes` command in [Command Palette](Command-Palette.md)
...
- `ComicBookUI.DefaultZoom` sets the first-open zoom for comic archives (`.cbz`, `.cbr`, …); empty uses Fit Page (the historical comic default), not the global `DefaultZoom`. Use `fit width` so new comics fill the window while PDFs stay at Fit Page (fixes #5946)
...
- bookmarks (PDF outline) entries are drawn with the color and the bold / italic style the document asks for
...
- Markdown documents have a **Show Generated HTML** command (`Ctrl + K`, `Show Generated HTML` command in [Command Palette](Command-Palette.md)) that saves the rendered HTML to a temporary `.html` file and opens it in Notepad
...
- With `SidebarWindowSize = grow` (default `keep` leaves the window alone, fixes #6205), showing the Bookmarks (or Favorites) sidebar grows the window when the work area has room, so the page keeps its size; hiding it shrinks the window again. Switching tabs does not change the window size. If the grown window would sit off-screen it is moved fully into view. If there is not enough room, the canvas shrinks as before (fixes #6080)
...
- Read Aloud: adjustable playback speed — pick it in the new `Speed` submenu (next to `Voice` in the Read Aloud menu, toolbar dropdown and context menu) or with the speed slider on the playback bar; the speed persists across sessions via the `ReadAloudSpeed` advanced setting
...
- document properties page size shows cm, mm, in (locale unit first) and points, e.g. `21.0x29.7 cm, 210x297 mm, 8.27x11.69 in, 595x842 pt` (fixes #2186, #5992)
...
- middle-click auto-scroll is now smooth: a high-frequency timer with fractional-pixel accumulation, so slow speeds stay even
...
- Save As warns when a file can't be written (e.g. the destination path exceeds the Windows `MAX_PATH` limit)
...
- home page document history: `Del` removes the keyboard-selected entry from history (shown as a shortcut on **Remove From History** in the right-click menu); the menu also has **Delete File** (recycle bin + drop from history) and **Show in folder** (Explorer, or Navigate Files in Folder when **File / Use SumatraPDF file picker** is on). The same **Show in folder** choice applies from File, the page context menu, and a tab's context menu
...
- implement the `[GetFileState]` [DDE command](DDE-Commands.md) to query a document's path, zoom, view mode and version
...
- `[SetView]` DDE command now accepts a zoom of `0` to keep the current zoom
- toggle commands (`CmdToggleFullscreen`, `CmdTogglePresentationMode`, `CmdToggleToolbar`, `CmdToggleMenuBar`, `CmdToggleContinuousView`, `CmdToggleTableOfContents`/`CmdToggleBookmarks`) accept an optional `state` argument to force an on/off state instead of toggling, e.g. `[CmdToggleFullscreen on]` (fixes #5067)
...
- add **Set Inverse Search Command Line** (`Ctrl + K`, `Set Inverse Search Command Line` command in [Command Palette](Command-Palette.md)): opens a standalone dialog to configure the SyncTeX inverse-search command (with detected TeX editors in a drop-down and a Help link to [LaTeX integration](LaTeX-integration.md)); OK saves `InverseSearchCmdLine` and enables TeX enhancements, Cancel leaves settings unchanged. Works from the home page without an open document
- one-click light/dark theme switching: **Toggle Light/Dark Theme** (`Ctrl + K`, `Toggle Light/Dark Theme` command in [Command Palette](Command-Palette.md)) flips between the last used light and dark themes (remembered in the new `LastLightTheme` / `LastDarkTheme` advanced settings). **Follow Windows** in the Change Theme dialog (or `Theme = System` in advanced settings) makes SumatraPDF follow the Windows light/dark app mode, switching automatically when the OS does (fixes #5995). Inspired by the [SumatraPDF Plus](https://github.com/dengxibo/sumatrapdf-plus) fork
...
- smarter inverted (dark) page rendering for MuPDF documents (PDF, XPS, EPUB, MOBI, FB2, HTML, etc.): the `DocumentColorsFollowTheme` advanced setting (also via `Ctrl + K`, `Set Document Colors Follow Theme` command in [Command Palette](Command-Palette.md) and the **Change Theme** dialog) picks how page colors follow the UI theme: `off` (keep original page colors, the default), `smart` (recolor text and background but not images), or `legacy` (recolor text, background and images — pre-3.7 behavior). **Invert Colors** / `Shift + I` swaps the page colors for the session and is not saved. **Toggle Preserve PDF Image Colors in Dark Mode** switches image preservation for the session. Ported from the [SumatraPDF Plus](https://github.com/dengxibo/sumatrapdf-plus) fork
- reflowable MuPDF documents (EPUB, HTML, FB2, MOBI, TXT) apply those page colors as user CSS, so images stay as in the file even in `legacy` mode and when inverting with `Shift + I`
...
- clearer rendering of CAD / engineering-drawing PDFs: hairline strokes get a zoom-aware minimum width and typical CAD-export grays are darkened toward Acrobat-like contrast, so drawings stay readable when zoomed out. Off by default. Set `EngineeringDrawingEnhance` to `auto` to apply when a drawing is detected (PDF/E marker, CAD authoring tool in the metadata, or content heuristics; screenshot/raster and WPS-style hairline exports are handled too), or `on` for every PDF; per document, **Toggle Engineering Drawing Enhancement** (`Ctrl + K`, `Toggle Engineering Drawing Enhancement` command in [Command Palette](Command-Palette.md)). See [CAD / Engineering Drawings](CAD-Engineering-Drawings.md). Ported from the [SumatraPDF Plus](https://github.com/dengxibo/sumatrapdf-plus) fork
...
- `CmdRedo` : "Redo" — re-apply the change Undo took back (`Ctrl + Shift + Z`)
...
- `CmdToggleHoverPreview` : "Toggle Citation Hover Preview" — palette-only; enables/disables the citation/reference hover popup (`CitationHoverDelay`)
...
- `CmdSetInverseSearch` : "Set Inverse Search Command Line" — opens a dialog to configure the SyncTeX inverse-search command (`Ctrl + K`, `Set Inverse Search Command Line` command in [Command Palette](Command-Palette.md))
...
- Arm 64-bit builds
- dark mode (menu `Settings / Theme` or `Ctrl + K`, `Select next theme` command in [Command Palette](Command-Palette.md))
