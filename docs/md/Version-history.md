...
- Annotations window: the list is as tall as the annotations (up to 12 rows) and keeps that height while you move between them. Extra window height goes to the Contents box. Each list row shows the type and contents on the left and the page number on the right. Per-annotation options (alignment, font, color, and so on) sit in a two-column table: right-aligned labels on the left, controls on the right. Contents stays a label above a full-width edit box
