...
- Showing the Bookmarks (or Favorites) sidebar grows the window when the work area has room, so the page keeps its size; hiding it shrinks the window again. If the grown window would sit off-screen it is moved fully into view. If there is not enough room, the canvas shrinks as before (fixes #6080)
