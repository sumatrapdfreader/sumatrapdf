...
- The floating selection toolbar uses compact SVG icons for every built-in action, sharing the PDF annotation toolbar’s icons for annotation actions. Its built-in buttons can be chosen and reordered with `SelectionToolbarLayout`, the same idea as `ToolbarCustomLayout` (discussion #6015)
