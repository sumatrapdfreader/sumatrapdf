...
- **Navigate Files in Folder** has Back, Forward, Up and Home buttons before the folder name. Back / Forward (`Alt + Left` / `Alt + Right`, also `Backspace`) walk the folders visited in that window, Up (`Alt + Up`) goes to the parent (from a drive root: to Home), Home lists all drives followed by Explorer's Quick access folders and files (fetched once; F5 there re-reads them) (#6232)
