...
- fix a half-drawn row at the bottom of the floating Find window's results when the window height isn't a whole number of rows: the cut-off row is now painted as plain background. Same for the Navigate Files in Folder list (fixes #5796)
...
- `CmdNavigateFilesInFolder` : "Navigate Files in Folder" (`Ctrl + Shift + Up`) — directory browser for openable files (stays open; Enter/double-click replaces the current tab or enters a directory, `..` goes up, `Del` deletes the selected file, `F5` refreshes, Esc closes)
