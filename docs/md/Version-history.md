...
- when Find highlights all matches, the current (active) match now uses your `FixedPageUI.SelectionColor` and the other matches use its inverse, so the match you're on stands out in the color you picked as most noticeable; previously the active match was a fixed orange and every other match shared the selection color (fixes #5740)
