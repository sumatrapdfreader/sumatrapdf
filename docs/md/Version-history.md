...
- rename `CmdDiscardAnnotations` to `CmdDiscardChanges` ("Discard Changes"): reloads the current document from disk, dropping unsaved annotations and form changes; available from the tab context menu when there are unsaved changes and with `Ctrl + K`, `Discard Changes` command in Command Palette
...
- Markdown documents have a **Show Generated HTML** command (`Ctrl + K`, `Show Generated HTML` command in Command Palette) that saves the rendered HTML to a temporary `.html` file and opens it in Notepad
...
- one-click light/dark theme switching: **Toggle Light/Dark Theme** (`Ctrl + K`, `Toggle Light/Dark Theme` command in Command Palette) flips between the last used light and dark themes (remembered in the new `LastLightTheme` / `LastDarkTheme` advanced settings). **Follow Windows** in the Change Theme dialog (or `Theme = System` in advanced settings) makes SumatraPDF follow the Windows light/dark app mode, switching automatically when the OS does (fixes #5995). Inspired by the [SumatraPDF Plus](https://github.com/dengxibo/sumatrapdf-plus) fork
...
- `CmdSetInverseSearch` : "Set Inverse Search Command Line" — opens a dialog to configure the SyncTeX inverse-search command (`Ctrl + K`, `Set Inverse Search Command Line` command in Command Palette)
...
- Arm 64-bit builds
- dark mode (menu `Settings / Theme` or `Ctrl + K`, `Select next theme` command in Command Palette)
