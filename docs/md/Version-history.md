...
- **Ctrl+Delete** deletes the annotation under the cursor, even while the annotations window is open and a different annotation is selected in the list
...
- The annotations window opens with annotations from the pages currently on screen; annotations from the rest of a large document load in the background without losing the current selection
...
- The home page has a command-palette button above the **?** keyboard-shortcuts button. The Keyboard Shortcuts window can be moved by dragging it
