...
SumatraPDF comes as an installer or as a portable single `.exe`. This page covers downloading, installing (interactive or silent), extracting without installing, where settings go, and fixing failed upgrades.

**Download the [latest release](https://www.sumatrapdfreader.org/download-free-pdf-viewer) installer and double-click it.** At a glance:

- **Installer:** `SumatraPDF-<ver>-install.exe`, registers file associations and optional preview handler.
- **Portable:** `SumatraPDF-<ver>.exe`, a single self-contained `.exe`.
- **Silent install:** `-install -silent`.
- **Extract only:** `-x -d <dir>`, no registration.
- **Custom settings directory:** `-appdata <directory>`.

## Download

- [latest release](https://www.sumatrapdfreader.org/download-free-pdf-viewer)
- [pre-release](https://www.sumatrapdfreader.org/prerelease)

SumatraPDF has two flavors: installer and portable.

**Portable** (`SumatraPDF-<ver>.exe` downloaded as `SumatraPDF-<ver>.zip`) is a single, self-contained `.exe`.

**Installer** (`SumatraPDF-<ver>-install.exe`) installs to `%LOCALAPPDATA%\SumatraPDF` (or `%PROGRAMFILES%` with `-all-users`), registers file associations, and optional preview handler.

## Install

- **Interactive:** double-click the installer.
...
## Use a custom settings directory (enterprise / RDP)
...
All users sharing that path use the same `SumatraPDF-settings.txt`. Ensure the directory is writable by every account that runs SumatraPDF. See [Portable vs installed settings](#portable-vs-installed-settings) for the default locations.
...
- Use `-install -silent` for unattended deployment; add `-log` to diagnose failures.
- Use `-x` to get matching `SumatraPDF.exe` and `libsumatrapdf.dll` without touching file associations.
- Close Explorer preview panes and stop Windows Search before upgrading if a DLL is locked.
- Don't run SumatraPDF as administrator for everyday reading.
...
- [Installer cmd-line arguments](Installer-cmd-line-arguments.md) — every installer option
