...
void pdf_freeimage(fz_image *fzimg)
{
...
{
	unsigned char *s = ind->p;
	unsigned char *d = pix->p;
	int i, k;
	for (i = 0; i < pix->w * pix->h; i++)
	{
...
pdf_loadtile(fz_image *img, fz_pixmap *tile)
{
	...
	if (src->indexed)
	{
		...
		error = fz_newpixmap(&tmp, tile->x, tile->y, tile->w, tile->h, 1, 0);
		if (error)
			return error;
		...
		for (y = 0; y < tile->h; y++)
		{
			for (x = 0; x < tile->w; x++)
			{
				tile->p[(y * tile->w + x) * tile->n] = 255;
				i = tmp->p[y * tile->w + x] / bpcfact;
				i = CLAMP(i, 0, src->indexed->high);
				for (k = 0; k < src->indexed->base->n; k++)
				{
					tile->p[(y * tile->w + x) * tile->n + k + 1] =
