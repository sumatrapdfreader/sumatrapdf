...
t3render(fz_glyph *glyph, fz_font *fzfont, int cid, fz_matrix trm)
{
    ...
    error = fz_newgraphics(&gc, GCMEM);
    if (error)
        return fz_rethrow(error, "cannot create graphics context");

    error = fz_drawtree(&pixmap, gc, tree, ctm, pdf_devicegray, bbox, 0);
    if (error)
    {
        fz_dropgraphics(gc);
        return fz_rethrow(error, "cannot render display tree");
    }
