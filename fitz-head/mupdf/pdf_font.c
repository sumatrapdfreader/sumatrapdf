...
ftrender(fz_glyph *glyph, fz_font *fzfont, int cid, fz_matrix trm)
{
    ...
    if (font->substitute && fzfont->wmode == 0)
    {
        ...
        fterr = FT_Load_Glyph(font->ftface, gid,
                FT_LOAD_NO_HINTING | FT_LOAD_NO_BITMAP | FT_LOAD_IGNORE_TRANSFORM);
        if (fterr)
            return fz_throw("freetype cannot load glyph %d: %s", gid, pdf_fterrorstring(fterr));
    ...
    if (fterr)
        fz_warn("freetype cannot load glyph %d: %s", gid, pdf_fterrorstring(fterr));
    ...
    if (fterr)
        fz_warn("freetype cannot render glyph %d: %s", gid, pdf_fterrorstring(fterr));

    glyph->w = face->glyph->bitmap.width;
    glyph->h = face->glyph->bitmap.rows;
    glyph->x = face->glyph->bitmap_left;
    glyph->y = face->glyph->bitmap_top - glyph->h;
    glyph->p = face->glyph->bitmap.buffer;

    for (y = 0; y < glyph->h / 2; y++)
    {
        for (x = 0; x < glyph->w; x++)
        {
            unsigned char a = glyph->p[y * glyph->w + x ];
            unsigned char b = glyph->p[(glyph->h - y - 1) * glyph->w + x];
            glyph->p[y * glyph->w + x ] = b;
            glyph->p[(glyph->h - y - 1) * glyph->w + x] = a;
        }
    }
...
loadsimplefont(pdf_font **fontp, pdf_xref *xref, fz_obj *dict, fz_obj *ref)
{
	...
	if (cmap)
	{
		...
		if (e)
		{
			error = fz_throw("freetype could not set cmap: %s", pdf_fterrorstring(e));
			goto cleanup;
		}
	}
	else
	...
	if (!etable)
        {
            error = fz_throw("outofmem: font encoding table");
            goto cleanup;
        }
...
loadcidfont(pdf_font **fontp, pdf_xref *xref, fz_obj *dict, fz_obj *ref, fz_obj *encoding, fz_obj *tounicode)
{
	...
	if (kind == TRUETYPE)
	{
		...
		else if (font->substitute)
		{
			...
			if (e)
				return fz_throw("freetype: cannot find unicode cmap when emulating CID font: %s", pdf_fterrorstring(e));
