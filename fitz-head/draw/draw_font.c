/* TODO: make the eviction logic round-robin so there are never any holes,
	and then throw out the LRU table.
 */
...
fz_renderglyph(fz_glyphcache *arena, fz_glyph *glyph, fz_font *font, int cid, fz_matrix ctm)
{
	...
	val->x = glyph->x;
	val->y = glyph->y;
	val->p = arena->buffer + arena->used;

	arena->used += size;

	memcpy(val->p, glyph->p, glyph->w * glyph->h);
	glyph->p = val->p;
