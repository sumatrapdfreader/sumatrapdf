...
fz_newfaxe(fz_filter **fp, fz_obj *params)
{
    ...
    if (!fax->ref) { fz_free(fax); return fz_throw("outofmem: fax reference buffer"); }

    fax->src = fz_malloc(fax->stride);
    if (!fax->src) { fz_free(fax); fz_free(fax->ref); return fz_throw("outofmem: fax scratch buffer"); }
