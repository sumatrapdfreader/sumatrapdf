...
static int iswhitenode(fz_solidnode *node)
{
	if (!strcmp(node->cs->name, "DeviceGray"))
		return fabs(node->p[0] - 1.0) < FLT_EPSILON;
	if (!strcmp(node->cs->name, "DeviceRGB"))
		return fabs(node->p[0] - 1.0) < FLT_EPSILON &&
				fabs(node->p[1] - 1.0) < FLT_EPSILON &&
				fabs(node->p[2] - 1.0) < FLT_EPSILON;
	if (!strcmp(node->cs->name, "DeviceCMYK"))
		return fabs(node->p[0]) < FLT_EPSILON &&
				fabs(node->p[1]) < FLT_EPSILON &&
				fabs(node->p[2]) < FLT_EPSILON &&
				fabs(node->p[3]) < FLT_EPSILON;
	return 0;
}
		...
		if (fz_isimagenode(current))
		{
			...
			if (image->w == 1 && image->h == 1)
			{
				...
				if (image->cs)
				{
					...
					for (i = 0; i < image->n + image->a; i++)
						v[i] = pix->p[i + 1] / 255.0;

					fz_droppixmap(pix);

					error = fz_newsolidnode(&color, image->cs, image->n, image->a, v);
					if (error)
						return error;
